...
public class InvoiceDocumentADA : IDocument
{
    ...
    public InvoiceDocumentADA(InvoiceModel model, IPriceToWords priceToWords, IInvoiceTexts texts)
    {
        Model = model;
    ...
    private void ComposeHeader(IContainer container)
    {
        ...
        container.Row(row =>
        {
            ...
            {
                column.Item().AlignCenter().Text($"{_texts.JobDoneAct()}").Style(titleStyle);
    ...
    private void ComposeReciever(IContainer container)
    {
        container.ShowEntire().PaddingTop(10).Column(column =>
        {
            column.Item().Row(row =>
            {
                row.ConstantItem(110).Text($"{_texts.Prepared()}: ");
            ...
            column.Item().Row(row =>
            {
                ...
                row.RelativeItem().AlignCenter()
                                    .Text($"{_texts.Contractor()}: {_texts.Name()}, {_texts.Surname()}, {_texts.Signature()}, {_texts.Date()}")
                                    .FontSize(8);
            });

            column.Item().Row(row =>
            {
                row.ConstantItem(110).Text($"{_texts.Coordinated()}: ");
            ...
            column.Item().Row(row =>
            {
                row.ConstantItem(110);
                row.RelativeItem().AlignCenter()
                                    .Text($"{_texts.ResponsibleWorker()}: {_texts.Position()}, {_texts.Name()}, {_texts.Surname()}, {_texts.Signature()}, {_texts.Date()}")
                                    .FontSize(8);
            });
        });
    }
